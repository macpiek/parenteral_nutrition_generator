...
(function (root, factory) {
  ...
  async function fetchTemplateBuffer ({ fetchFn, templateFile }) {
    ...
    if (!response?.ok) {
      ...
      throw new Error(`Nie udało się pobrać szablonu XLSX:${status}${statusText}.`);
    }
  ...
  async function applyPrintArea ({ buffer, worksheetName, printRange, JSZipImpl }) {
    ...
    if (!clearedXml.includes("</workbook>")) {
      throw new Error("Nieprawidłowa struktura xl/workbook.xml w szablonie XLSX.");
    }
    ...
    if (!finalXml.includes(printAreaNode)) {
      throw new Error("Nie udało się ustawić obszaru wydruku w szablonie XLSX.");
    }
